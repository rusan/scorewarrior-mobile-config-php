...
* **Fixtures cache**: in-memory, ключом служат `kind+platform`. Инвалидация по изменению `mtime` файлов; счётчик `gen` увеличивается при обновлении, чтобы инвалидировать зависящие кэши.
* **Resolver cache**: кэш результата подбора для комбинации параметров + текущий `gen` фикстур.
* **Memory cache**: используется встроенное кеширование в памяти (APCu) для быстрого доступа к данным.
* **LRU Cache**: локальный кэш с автоматическим вытеснением наименее используемых элементов для предотвращения утечек памяти. Лимит настраивается через `LOCAL_CACHE_MAX_SIZE` (по умолчанию: 1000 элементов).
